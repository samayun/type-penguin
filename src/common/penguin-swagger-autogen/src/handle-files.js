...
function readEndpointFile(
  ...
  return new Promise((resolve) => {
    ...
    return fs.readFile(filePath, 'utf8', async function (err, data) {
      if (err || data.trim() === '') {
      ...
      if (regex != '') {
        ...
        for (let idxElem = 0; idxElem < aData.length; idxElem++) {
           ...
           */
          console.log('Handling endpoint functions...', endpointFunctions);
      ...
      if (aRoutes && aRoutes.length >= 1) {
        for (let file = 0; file < aRoutes.length; file++) {
          ...
          let exportPath = null;
          const rexRequire = /\s*require\s*\n*\t*\(/;
          console.log(`PASS REGEX `, rexRequire.test(data));
          ...
          listOfFileName = [...listOfFileName];
          console.log(`listOfFileName: ${listOfFileName}`);
      ...
      console.log({ paths, aRoutes, allPaths, filePath, relativePath });
 ...
 */
// FIXME: parse our dist js files using regex
async function getImportedFiles(data, relativePath, lang = 'ts') {
  let importedFiles = [];
  try {
    let importeds;
    let requireds;
